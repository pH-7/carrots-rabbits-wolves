...
export class World {
  ...
  getPosition(element, position) {
    return element.filter((element) => this.arePositionsIdentical(element.position, position));
  }

  getDistance(positionA, positionB) {
    return Math.abs(positionA.horizontal - positionB.horizontal) + Math.abs(positionA.vertical - positionB.vertical);
  }
