...
export class WorldSimulation extends World {
    ...
    for (let horizontal = 0; horizontal < WORLD.width; horizontal++) {
      for (let vertical = 0; vertical < WORLD.height; vertical++) {
        if (!map[horizontal][vertical].length) {
          const foodFrequency = randomInteger(0, WORLD.foodDensity) === WORLD.foodDensity;
          if (foodFrequency) {
  ...
  createWolves() {
    const possiblePositions = arrayShuffle(this.generateMapPositions(WORLD.width, WORLD.height));

    const position = possiblePositions.slice(0, randomInteger(1, WORLD.maxTiles));
